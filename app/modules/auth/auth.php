...
if (isset($_GET['do']) && $_GET['do'] == 'signout' && isset($_GET['hash'])) {
    ...
    if ($logged_in['flag'] && $hash == $_SESSION['authID']) {
        $signout = $auth->signout($hash);
    }
...
#Google
    if ($_GET['oauth'] == 'google') {
        ...
        foreach ($google as $arr => $val) {
            if ($val != '') {
                if ($arr == 'email') {
                    $special_fields['oauth_email'] = $val;
                }
                if ($arr == 'name') {
                    $special_fields['first_name'] = $val;
                }
                if ($arr == 'link') {
                    $special_fields['google_link'] = $val;
                }
                if ($arr == 'gender') {
                    $special_fields['gender'] = $val;
                }
            }
        }
...
#Twitter
    if ($_GET['oauth'] == 'twitter') {
        ...
        if (!isset($_SESSION['twitter_user_data'])) {
            header('location: https://' . $_SERVER['HTTP_HOST'] . '/signin/');
            exit;
        }
        ...
        foreach ($twitter as $arr => $val) {
            if ($val != '') {
                ...
                if ($arr == 'name') {
                    $special_fields['twitter_name'] = $val;
                }
                if ($arr == 'screen_name') {
                    $special_fields['twitter_username'] = $val;
                }
                if ($arr == 'location') {
                    $special_fields['twitter_location'] = $val;
                }
                if ($arr == 'url') {
                    $special_fields['twitter_website'] = $val;
                }
                if ($arr == 'followers') {
                    $special_fields['twitter_followers'] = $val;
                }
                if ($arr == 'friends') {
                    $special_fields['twitter_friends'] = $val;
                }
                if ($arr == 'created_at') {
                    $special_fields['twitter_created_at'] = $val;
                }
                if ($arr == 'lang') {
                    $special_fields['twitter_lang'] = $val;
                }
                if ($arr == 'time_zone') {
                    $special_fields['twitter_time_zone'] = $val;
                }
                if ($arr == 'verified') {
                    $special_fields['twitter_verified'] = $val;
                }
                if ($arr == 'profile_image_url_https') {
                    $special_fields['twitter_image'] = $val;
                }
            }
        }
    ...
    if ($_GET['oauth'] == 'facebook') {
        ...
        foreach ($facebook as $arr => $val) {
            if ($val != '') {
                ...
                if ($arr == 'last_name') {
                    $special_fields['last_name'] = $val;
                }
                if ($arr == 'birthday') {
                    $special_fields['facebook_birthday'] = $val;
                }
                if ($arr == 'website') {
                    $special_fields['facebook_website'] = $val;
                }
                if ($arr == 'gender') {
                    $special_fields['gender'] = $val;
                }
...
#Если пользователь авторизован, соединяем новые данные с аккаунтом пользователя
    if ($logged_in['flag']) {
        ...
        if (is_array($special_fields) && count($special_fields) > 0) {
            ...
            foreach ($special_fields as $paramKey => $paramValue) {
...
#Если пользователь не зарегистрирован, регистрируем
    ...
    if ($query->rowCount() == 0) {
        ...
        if ($signup_oauth['status'] != 'success') {
            answer_json($signup_oauth['status'], $signup_oauth['message']);
        }
...
#Двухфакторная авторизация
        if ($_POST['do'] == 'two_factor' && $_SESSION['two_factor'] != '') {
            ...
            if (strlen($two_factor_code) != 6 || !is_numeric($two_factor_code) || !$ga->verifyCode($user['two_factor'], $two_factor_code)) {
                answer_json('error', $lng['two_factor_code_error']);
            } else {
                $_SESSION['two_factor'] = '';
                answer_json('success', '');
            }
...
//        $recaptcha = filter_input(INPUT_POST, 'g-recaptcha-response', FILTER_SANITIZE_STRING);
        ...
        if (((strlen($ethereum) != 42 || substr($ethereum, 0, 2) != '0x') && $ethereum != '') || $product['ethereum_field_required'] == '1') {
            answer_json('ethereum', $lng['error_invalid_ethereum']);
        }
        if ($agree != 1 && $product['user_agreement'] != '') {
            answer_json('agree', 'please_agree');
        }

//        if ($product['captcha_enabled'] == '1' && (!isset($_SESSION['captcha_answer']) || $captcha != $_SESSION['captcha_answer'])) {
//            unset($_SESSION['captcha_answer']);
//            answer_json('captcha', $lng['error_captcha']);
//        }
...
//        answer_json('captcha', $signup['message']);
        if ($signup['status'] != 'success') {
            answer_json($signup['status'], $signup['message']);
        } else {
...
#Отправка email
                ...
                if ($sendmail['status'] == 'error') {
                    $auth->request_delete($signup['request_id']);
                    answer_json('error', 'Email - ' . $lng['error_system']);
                }
...
#Вход после регистрации
            if ($product['email_activation'] == '0') {
                ...
                if ($signin['status'] != 'success') {
                    answer_json($signin['status'], $signin['message']);
                } else {
                    answer_json($signin['status'], '');
                }
            }
...
#Запрос ключа восстановления для смены пароля
    if ($_POST['do'] == 'reset') {
        ...
        if ($password_reset_request['status'] != 'success') {
            answer_json($password_reset_request['status'], $password_reset_request['message']);
        } else {
...
#Отправка email
            ...
            if ($sendmail['status'] == 'error') {
                $auth->request_delete($password_reset_request['request_id']);
                answer_json('error', 'Email - ' . $lng['error_system']);
            }
...
#Запрос повторного письма для регистрации
    if ($_POST['do'] == 'activation_resend') {
        ...
        if ($activation_resend['status'] != 'success') {
            if ($activation_resend['status'] == 'time') {
                answer_json('time', ($activation_resend['time'] * 1000));
            } else {
                answer_json('error', '');
            }
        }
...
#Отправка email
        ...
        if ($sendmail['status'] == 'error') {
            $auth->request_delete($activation_resend['request_id']);
            answer_json('error', 'Email - ' . $lng['error_system']);
        }
...
#Смена пароля с помощью ключа восстановления
    if ($_POST['do'] == 'reset_code') {
        ...
        if ($code == '' || $request_id == '' || !is_numeric($request_id)) {
            answer_json('error', 'invalid_code');
        }
        $request_get = $auth->request_get($code, $request_id, 'reset');
        if ($request_get['status'] != 'success') {
            answer_json('error', $request_get['message']);
        }

        $password_reset = $auth->password_reset($code, $request_id, $new_password, $new_password_confirmation);
        if ($password_reset['status'] != 'success') {
            answer_json($password_reset['status'], $password_reset['message']);
        }
